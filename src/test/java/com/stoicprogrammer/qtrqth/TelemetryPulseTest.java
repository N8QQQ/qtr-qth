...
@Override void given_trigger(final String s) { this.trigger = s; }
@Override void given_starting_pulse() { this.pulse = TelemetryPulse.start(trigger, mockNtp, com.stoicprogrammer.qtrqth.model.ConfluenceHealth.HEALTHY_HARDWARE, MOCK_TIME, sampleData, PrecisionMetrics.EMPTY); }
@Override void when_starting_pulse() { this.pulse = TelemetryPulse.start(trigger, mockNtp, com.stoicprogrammer.qtrqth.model.ConfluenceHealth.HEALTHY_HARDWARE, MOCK_TIME, sampleData, PrecisionMetrics.EMPTY); }
@Override void when_logging_final() { pulse.logFinal(mockLogger); }
@Override void then_id_is_not_empty() { assertThat(pulse.pulseId()).isNotNull().isNotEmpty(); }
@Override void then_ingress_time_is_frozen() { assertThat(pulse.ingressTime()).isEqualTo(MOCK_TIME); }
@Override void then_logger_was_called() { verify(mockLogger, atLeastOnce()).info(anyString(), any(), any(), any(), any()); }
}
}
